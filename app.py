...
def get_api_key():
    try:
        ...
        st.error("⚠️ API key not configured in .streamlit/secrets.toml")
        return None
...
def get_ai_response(prompt, intent, conversation_context=None, model='gemini-2.0-flash-exp'):
    """Generate AI response using Gemini API"""
    url = f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={API_KEY}"
    
    # System prompts based on intent
    system_prompts = {
        'sip': "Focus on SIP benefits, how to start, recommended amounts, and best funds for beginners.",
        'tax': "Explain tax-saving options like 80C, ELSS, PPF, NPS with clear comparisons.",
        'stocks': "Discuss stock market basics, index funds vs individual stocks, and risk management.",
        'retirement': "Guide on retirement planning, corpus calculation, and investment strategies.",
        'risk': "Help assess risk tolerance and recommend appropriate investment products.",
        'debt': "Advise on loan management, EMI calculations, and debt reduction strategies.",
        'budget': "Suggest budgeting frameworks like 50-30-20, expense tracking tips.",
        'insurance': "Explain term insurance, health insurance, and why to separate investment from insurance.",
        'mutual_fund': "Compare equity, debt, and hybrid funds with risk-return profiles.",
        'emergency': "Stress importance of emergency fund, ideal amount (6-12 months), and liquid fund options.",
        'general': "Understand the user's financial situation and provide personalized guidance."
    }
    context_hint = system_prompts.get(intent, system_prompts['general'])
    
    # System instruction
    system_instruction = f"""You are Finny, a warm and knowledgeable financial advisor for Indian markets. You're like that smart friend who actually knows about money!
...
- Use examples: "Think of SIP like a gym membership for your wealth..."
- Show personality: occasional emojis, light humor, enthusiastic tone
- Ask follow-ups: "What's your current age?" "Do you have an emergency fund?"
...
Respond naturally as Finny would - warm, helpful, and conversational!"""
    ...
    # Request payload - FIXED structure
    data = {
        "contents": contents,
        "systemInstruction": {
...
def generate_response(query):
    ...
    if API_KEY:
        ...
        return get_ai_response(query, intent, conversation_context=conversation_context)
    else:
...
with st.sidebar:
    ...
    st.markdown("---")
    st.success("🤖 AI Enabled" if API_KEY else "🤖 AI Unavailable")
...
if tool == "💬 Chat":
    ...
    st.info("💡 **Try asking:** 'I'm 25 and want to start investing, where should I begin?' • 'How can I reduce my tax burden?' • 'What's your take on investing in gold?'")
